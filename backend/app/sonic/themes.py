...
def _affinity_score(theme: Theme, sky: SkyVector) -> float:
    ...
    raw_score = dot / weight_total

    # Physical weather realism gate: dense radiation fog ("Visibility 300 metres. No hurry.")
    # cannot persist under strong wind gusts (> 10-12 km/h scatter), high daytime sun,
    # or moderate/broken cloud decks.
    if theme.id == "nordic_fog":
        if sky.gust_variance > 0.22 or sky.sun_elevation > 0.22 or sky.cloud_depth < 0.82:
            raw_score -= 0.45
