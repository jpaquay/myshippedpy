...
class SpotifyAuth:
    ...
    def configured(self) -> bool:
        ...
        cid = getattr(self._settings, "spotify_client_id", None)
        return bool(cid and not str(cid).startswith("dev-placeholder"))
    ...
    async def exchange_code(
        ...
        *,
        ...
        sec = getattr(self._settings, "spotify_client_secret", None)
        if sec is not None:
            raw_sec = sec.get_secret_value() if hasattr(sec, "get_secret_value") else str(sec)
            if raw_sec and not raw_sec.startswith("dev-placeholder"):
                form["client_secret"] = raw_sec
