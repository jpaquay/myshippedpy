...
class Container:
    ...
    def weather(self) -> WeatherSource:
        def real() -> WeatherSource:
            ...
            # Keyword-only: passing settings positionally raises, and because
            # this factory is wrapped in a try/except that would have silently
            # demoted every production deploy to fixture weather.
            return OpenMeteoSource(settings=self.settings)
    ...
    def oracle(self) -> AcousticOracle:
        def real() -> AcousticOracle:
            if not self.settings.has_lastfm:
                raise RuntimeError("no Last.fm API key — theme-only mode")
    ...
    def sinks(self) -> list[PlaylistSink]:
        ...
        def real() -> list[PlaylistSink]:
            ...
            if self.settings.has_spotify:
                ...
                out.append(SpotifySink(self.settings))
            out.append(M3USink())
            return out
