...
class Settings(BaseSettings):
    ...
    def has_lastfm(self) -> bool:
        return (
            bool(self.lastfm_api_key and not self.lastfm_api_key.startswith("dev-placeholder"))
            and self.lastfm_enabled
        )
    ...
    def has_spotify(self) -> bool:
        return bool(
            ...
            and not self.spotify_client_id.startswith("dev-placeholder")
            and self.spotify_client_secret
            and not self.spotify_client_secret.startswith("dev-placeholder")
        )
