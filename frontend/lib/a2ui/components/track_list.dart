...
class _TrackRowState extends ConsumerState<_TrackRow> {
  ...
  Widget build(BuildContext context) {
    ...
    final String? spotifyUri = asStringOrNull(inner['spotify_uri']);
    final String? lastfmUrl = asStringOrNull(inner['lastfm_url']);
    final String? trackKey =
        asStringOrNull(scored['track_key']) ?? spotifyUri ?? lastfmUrl;

    final ActivePlayerTrack? active = ref.watch(activePlayerTrackProvider);
    final bool isPlayingThis =
        active != null && active.title == title && active.artist == artist;
    ...
    return Container(
      color: isPlayingThis
          ...
          : (isPeak ? BgPalette.gold50.withValues(alpha: 0.55) : null),
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: <Widget>[
          InkWell(
                ...
                children: <Widget>[
                  ...
                  IconButton(
                    icon: Icon(
                      ...
                      color: isPlayingThis ? colors.primary : colors.onSurfaceVariant,
                      size: 26,
                    ),
                    tooltip: 'Play in Barogroove Deck',
                    onPressed: () {
                      ...
                      children: <Widget>[
                        Text(
                          ...
                          style: text.titleMedium?.copyWith(
                            fontWeight: isPeak || isPlayingThis
                                ? FontWeight.w700
                                : FontWeight.w600,
                            color: isPlayingThis ? colors.primary : null,
                          ),
                          maxLines: 1,
