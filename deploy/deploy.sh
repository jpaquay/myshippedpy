...
readonly -a SA_ROLES=(
  ...
  "roles/secretmanager.secretAccessor"    # read secret VALUES (not manage)
...
prompt_secret_version() {
  ...
  if [[ "${secret}" == "barogroove-token-encryption-key" ]]; then
    ...
    read -r answer || true
    if [[ ! "${answer}" =~ ^[Nn] ]]; then
      # Generated locally. Never printed, never logged, never stored on disk.
      value="$(python3 -c 'from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())' 2>/dev/null || true)"
      if [[ -z "${value}" ]]; then
        warn "could not generate a key (is 'cryptography' installed for python3?)"
      else
        info "generated a 32-byte Fernet key (not displayed)"
      fi
    fi
  fi

  if [[ -z "${value}" ]]; then
    printf '\n    Enter value for %s%s%s (input hidden, empty to skip): ' \
      "${C_BOLD}" "${secret}" "${C_RESET}"
    read -rs value || true
    printf '\n'
  fi
  ...
  if [[ -z "${value}" ]]; then
    warn "skipped ${secret} -- the service will not start until it has a value"
    warn "  add one later:  gcloud secrets versions add ${secret} --data-file=- --project ${PROJECT_ID}"
    return 0
  fi
...
build_and_deploy() {
  ...
  info "submitting build; this typically takes 3-6 minutes"

  run gcloud builds submit \
    --config deploy/cloudbuild.yaml \
    --region "${REGION}" \
    --project "${PROJECT_ID}" \
    --substitutions="_REGION=${REGION},_REPOSITORY=${REPOSITORY},_SERVICE=${SERVICE},_PROJECT_NUMBER=${PROJECT_NUMBER}" \
    .
  ...
  info "allowing unauthenticated invocation (auth is enforced in-app)"
...
verify() {
  ...
  for attempt in 1 2 3 4 5; do
    if curl -fsS --max-time 10 "${url}/healthz" >/dev/null 2>&1; then
      ok "/healthz responded (attempt ${attempt})"
  ...
  warn "/healthz never answered. Check the logs:"
