...
| `roles/secretmanager.secretAccessor` | Read secret **values** at instance start. Cloud Run resolves the `secretKeyRef` env vars using this identity before the container starts. | The instance does not start at all. Cloud Run treats a failed env-var secret resolution as a fatal startup error, so the revision never becomes ready. |
